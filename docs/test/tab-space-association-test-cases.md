...
- **G1 - Deleting a bookmark/pinned site with its tab open** is expected to currently orphan the tab (tab keeps running but disappears from both lists). Not fixed yet - confirm it still reproduces, don't be surprised by it.
...
| 4 | Reopen the sidebar | Tab reflects its ungrouped state correctly |

### A.5c Pinned tab: native "Remove from group" / ungroup
| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Click a pinned site to open its tab | Pinned icon shows "loaded" |
| 2 | With sidebar open, right-click the tab in Chrome's tab strip → "Remove from group" | Pinned icon should keep tracking the tab regardless of group (pinned tabs aren't grouped by association logic) |
| 3 | Close the sidebar panel, repeat with another pinned tab | - |
| 4 | Reopen the sidebar | Pinned icon still shows "loaded"; clicking it re-activates the same tab |
...
| 4 | Repeat steps 1-2, then move the tab using sidebar method (b) "Move to Space" instead | Same result as step 3 - confirm both entry points behave the same |
...
### B.5 Delete a bookmark/pinned site while its tab is open (known gap G1)
| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Open a bookmark's tab | Bookmark shows "loaded" |
| 2 | Delete that bookmark (not the tab) via the bookmark tree's delete option | **Per G1, expect**: the tab keeps running, but disappears from *both* the bookmark tree (node gone) and the regular tab list (still counted as "managed") - confirm this reproduces and note whether the tab is reachable at all from the sidebar afterward |
| 3 | Repeat for a pinned site | Same expected gap |
...
| 4 | Reopen the sidebar | Compare against B.5 step 2 - confirm whether deleting via Chrome's native manager (instead of the extension's own tree) while the sidebar was closed produces the same orphaned-tab gap, or something different |
| 5 | Repeat for a pinned site (delete via `chrome://bookmarks` while sidebar closed) | Same comparison as step 4 |
...
| 3 | Close the active tab, letting Chrome activate another tab in the *same* space | No scroll (no space change) | No scroll (no space change - this was the original "close tab causes spurious scroll" bug, confirm it's still fixed) | Scrolls to show the newly-activated tab |
