...
### Files to modify
- `src/components/Toast.tsx` -- UI redesign + undo button
- `src/hooks/useBookmarks.ts` -- snapshot before `removeTree`, expose undo callback
- `src/hooks/useTabs.ts` -- snapshot before `remove`, expose undo callback
- `src/components/BookmarkTree.tsx` -- pass undo callback to toast, remove ConfirmDeleteDialog usage for bookmark deletion
- `src/components/TabList.tsx` -- pass undo callback to toast
- `src/App.tsx` -- update toast state management to support undo callbacks
- `src/contexts/BookmarkTabsContext.tsx` -- bookmark tab close undo support
- `src/contexts/SpacesContext.tsx` -- close-all-tabs-in-space undo support
